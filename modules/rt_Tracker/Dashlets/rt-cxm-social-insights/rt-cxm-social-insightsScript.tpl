...
<script type="text/javascript">
    ...
    initialize: function (options) {
        ...
        $.ajax({
            ...
            url: 'module=rt_Tracker&action=functions&method=getBean&module=contacts&sugar_body_only=true',
            success: function (bean) { 
                this.context.set('collection', bean);
            },
            error: function(e){
                console.log(e);
                configError(e);
    ...
    setData: function (tab, callback, id) {
        ...
        if (that.sData[that.email] != undefined) {
            ...
            var emailPromise = $.ajax({
                ...
                crossDomain: true,
                timeout: 5000, //work around for jsonp not returning errors
                url: 'module=rt_Tracker&action=functions&method=fetchEmailBean&data='+window.btoa(JSON.stringify(data))+'&sugar_body_only=true'
            });
        }
        emailPromise.done(function (data) {
            ...
            if (that.sData[that.email]) {
                ...
                var decodePromise = $.ajax({
                    type: "GET",
                    dataType: 'jsonp',
                    crossDomain: true,
                    timeout: 5000, //work around for jsonp not returning errors
                    url: 'module=rt_Tracker&action=functions&method=trackDecode&data='+window.btoa(JSON.stringify(data))+'&sugar_body_only=true'
                });
			...
			if (typeof e.responseText != 'undefined' && !_.isEmpty(e.responseText)) {
				...
				that.social_status += (e.responseText.indexOf('LBL_') != -1) ? "{$MOD." + e.responseText + "}" : e.responseText;
